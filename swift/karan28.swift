...
class graph{
    ...
    //The error is probably here. You can use the original nodeList index and assign false to all vertices initially
...
func test(){
    let myGraph = graph([(1, 0), (0, 2), (1, 2), (0, 3), (0, 4), (3, 4)])
    let eulerianArray: [node] = myGraph.EulerianTour()!
    print(myGraph.nodeList.map({$0.identifier}))
    print(eulerianArray.map({$0.identifier}))
}
test()
